...
Design for ~1 year of hourly data:
...
- A `gap` of `horizon` hours is left between the training window and each
...
def rolling_origin_folds(X: pd.DataFrame, Y: pd.DataFrame, horizon: int,
    ...
    """Yield expanding-window folds with a leakage gap of `horizon` hours.

    X, Y must share a DatetimeIndex of forecast-issue times (as produced by
    build_supervised). Targets of issue time t span (t, t+horizon].
    """
